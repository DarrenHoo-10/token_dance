...
const [rust, go, ts, uploaderBridge, canonicalHandler] = await Promise.all([
  ...
  read("collector/crates/uploader/src/transport.rs"),
  read("server/internal/httpapi/handlers.go")
]);
...
assert.ok(uploaderBridge.includes("flatten_claimed_event"), "collector uploader must bridge the strict protocol into the canonical ingest shape");
assert.ok(uploaderBridge.includes('const INGEST_PATH: &str = "/v1/telemetry/batches"'), "collector and server ingest paths must match");
assert.ok(canonicalHandler.includes("type TelemetryEventInput struct"), "canonical Go server must expose telemetry ingestion");
assert.ok(canonicalHandler.includes("maxTelemetryEvents    = 1000"), "canonical server must accept the collector's 500-event maximum");
