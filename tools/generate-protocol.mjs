...
const needsExplicitJsonName = fieldName => fieldName.includes("_") || fieldName.startsWith("$");
...
function fieldType(raw, primitives, arrayPrefix, arraySuffix) {
  const { array, base, constValue } = parseType(raw);
  let mapped = base === "const"
    ? (primitives === tsPrimitive ? JSON.stringify(constValue) : primitives === goPrimitive ? "EventType" : "EventType")
    : (primitives[base] ?? base);
  if (array) mapped = `${arrayPrefix}${mapped}${arraySuffix}`;
  return mapped;
}

function buildDefs(spec) {
  const defs = {};
  ...
  for (const [name, fields] of Object.entries(spec.objects)) {
    ...
    if (name === "TelemetryEventsRequest") properties.events.maxItems = spec.defaults?.maxBatchEvents ?? 500;
    if (name === "TelemetryCapabilities") {
      properties.maxBatchEvents.maximum = spec.defaults?.maxBatchEvents ?? 500;
      properties.maxBatchBytes.maximum = spec.defaults?.maxBatchBytes ?? 1048576;
    }
...
function applyV1EnvelopeRules(defs) {
...
function applyV2EnvelopeRules(defs, spec) {
  defs.EventEnvelope.allOf = [
    {
      if: { type: "object", properties: { eventType: { const: "model_usage_recorded" } }, required: ["eventType"] },
      then: { required: ["model"], properties: { payload: { type: "object", required: ["usage", "meta"] } } }
    },
    {
      if: { type: "object", properties: { eventType: { const: "cost_recorded" } }, required: ["eventType"] },
      then: { properties: { payload: { type: "object", required: ["cost", "meta"] } } }
    },
    {
      if: { type: "object", properties: { eventType: { const: "skill_invoked" } }, required: ["eventType"] },
      then: { required: ["skill"], properties: { payload: { type: "object", required: ["activity", "meta"] } } }
    },
    {
      if: { type: "object", properties: { eventType: { const: "code_changed" } }, required: ["eventType"] },
      then: { properties: { payload: { type: "object", required: ["code", "meta"] } } }
    },
    {
      if: { type: "object", properties: { schemaVersion: { type: "integer" } }, required: ["schemaVersion"] },
      then: { properties: { schemaVersion: { const: spec.schemaVersion } } }
    }
  ];
  defs.TelemetryEventsRequest.properties.protocolVersion.const = spec.protocolVersion;
  defs.TelemetryEventsRequest.properties.events.maxItems = spec.defaults.maxBatchEvents;
}

function generateTypeScript(spec, versionLabel) {
  let ts = `// Generated by tools/generate-protocol.mjs from schemas/protocol/${versionLabel}/spec.json.\n// Do not edit manually.\n\nexport const PROTOCOL_VERSION = ${JSON.stringify(spec.version)} as const;\n`;
  if (spec.protocolVersion != null) {
    ts += `export const PROTOCOL_VERSION_NUMBER = ${spec.protocolVersion} as const;\n`;
    ts += `export const SCHEMA_VERSION = ${spec.schemaVersion} as const;\n`;
    ts += `export const METRIC_SEMANTICS_VERSION = ${spec.metricSemanticsVersion} as const;\n`;
    ts += `export const DEFAULT_MAX_BATCH_EVENTS = ${spec.defaults.maxBatchEvents} as const;\n`;
    ts += `export const DEFAULT_MAX_BATCH_BYTES = ${spec.defaults.maxBatchBytes} as const;\n`;
  }
  ts += `export type UInt64String = string;\nexport type DecimalString = string;\nexport type HmacSha256 = string;\nexport type Base64Url32 = string;\n\n`;
  ...
  for (const [name, members] of Object.entries(spec.unions ?? {})) {
    ...
    ts += `export type ${name} = ${members.join(" | ")}\n\n`;
  }
  return ts;
}

function generateRust(spec, versionLabel, unionMembers) {
  let rust = `// Generated by tools/generate-protocol.mjs from schemas/protocol/${versionLabel}/spec.json.\n// Do not edit manually.\n\nuse serde::{Deserialize, Serialize};\n\npub const PROTOCOL_VERSION: &str = ${JSON.stringify(spec.version)};\n`;
  if (spec.protocolVersion != null) {
    rust += `pub const PROTOCOL_VERSION_NUMBER: u32 = ${spec.protocolVersion};\n`;
    rust += `pub const SCHEMA_VERSION: u32 = ${spec.schemaVersion};\n`;
    rust += `pub const METRIC_SEMANTICS_VERSION: u32 = ${spec.metricSemanticsVersion};\n`;
    rust += `pub const DEFAULT_MAX_BATCH_EVENTS: u32 = ${spec.defaults.maxBatchEvents};\n`;
    rust += `pub const DEFAULT_MAX_BATCH_BYTES: u32 = ${spec.defaults.maxBatchBytes};\n`;
  }
  rust += `pub type UInt64String = String;\npub type DecimalString = String;\npub type HmacSha256 = String;\npub type Base64Url32 = String;\n\n`;
  for (const [name, values] of Object.entries(spec.enums)) {
  ...
  for (const [name, fields] of Object.entries(spec.objects)) {
    ...
    for (const [rawName, rawType] of Object.entries(fields)) {
      ...
      if (needsExplicitJsonName(fieldName)) {
        rust += `    #[serde(rename = ${JSON.stringify(fieldName)})]\n`;
      }
...
function generateGo(spec, versionLabel, packageName) {
  let go = `// Code generated by tools/generate-protocol.mjs; DO NOT EDIT.\n\npackage ${packageName}\n\n`;
  const hasUnions = Object.values(spec.unions ?? {}).some(members => members.length);
  if (hasUnions) {
    go += `import (\n\t"bytes"\n\t"encoding/json"\n\t"fmt"\n\t"io"\n)\n\n`;
  }
  go += `const ProtocolVersion = ${JSON.stringify(spec.version)}\n`;
  if (spec.protocolVersion != null) {
    go += `const ProtocolVersionNumber uint32 = ${spec.protocolVersion}\n`;
    go += `const SchemaVersion uint32 = ${spec.schemaVersion}\n`;
    go += `const MetricSemanticsVersion uint32 = ${spec.metricSemanticsVersion}\n`;
    go += `const DefaultMaxBatchEvents uint32 = ${spec.defaults.maxBatchEvents}\n`;
    go += `const DefaultMaxBatchBytes uint32 = ${spec.defaults.maxBatchBytes}\n`;
  }
  ...
  if (gofmt.status === 0) return gofmt.stdout;
  if (gofmt.error?.code !== "ENOENT") throw new Error(gofmt.stderr || "gofmt failed");
  return go;
}

async function generateVersion(versionLabel) {
  const specPath = path.join(root, "schemas", "protocol", versionLabel, "spec.json");
  const spec = JSON.parse(await readFile(specPath, "utf8"));
  const unionMembers = new Set(Object.values(spec.unions ?? {}).flat());
  const defs = buildDefs(spec);
  if (versionLabel === "v1") applyV1EnvelopeRules(defs);
  if (versionLabel === "v2") applyV2EnvelopeRules(defs, spec);

  const schemaIdBase = `https://schemas.tokenshow.dev/protocol/${versionLabel}`;
  const baseSchema = {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: `${schemaIdBase}.json`,
    title: `TokenShow Collector Protocol ${versionLabel}`,
    oneOf: spec.roots.map(name => ({ $ref: `#/$defs/${name}` })),
    $defs: defs
  };

  const outputs = [];
  outputs.push([path.join(root, "schemas", "protocol", versionLabel, "protocol.schema.json"), JSON.stringify(baseSchema, null, 2) + "\n"]);

  const ts = generateTypeScript(spec, versionLabel);
  const rust = generateRust(spec, versionLabel, unionMembers);
  const goPackage = versionLabel === "v1" ? "protocol" : "v2";
  const go = generateGo(spec, versionLabel, goPackage);

  if (versionLabel === "v1") {
    outputs.push([path.join(root, "web", "src", "protocol", "generated.ts"), ts]);
    outputs.push([path.join(root, "collector", "crates", "protocol", "src", "generated.rs"), rust]);
    outputs.push([path.join(root, "server", "internal", "protocol", "generated.go"), go]);
  } else {
    outputs.push([path.join(root, "web", "src", "protocol", "v2", "generated.ts"), ts]);
    outputs.push([path.join(root, "collector", "crates", "protocol", "src", "v2", "generated.rs"), rust]);
    outputs.push([path.join(root, "server", "internal", "protocol", "v2", "generated.go"), go]);
  }
  ...
  for (const rootName of spec.roots) {
    ...
    const rootSchema = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: `${schemaIdBase}/${fileName}`,
      $ref: `#/$defs/${rootName}`,
      $defs: defs
    };
    const content = JSON.stringify(rootSchema, null, 2) + "\n";
    rootSchemaContent.set(rootName, content);
    outputs.push([path.join(root, "schemas", "protocol", versionLabel, fileName), content]);
  }
  ...
  return { version: spec.version, count: outputs.length, stale };
}

const results = [];
for (const versionLabel of ["v1", "v2"]) {
  results.push({ versionLabel, ...(await generateVersion(versionLabel)) });
}

const stale = results.flatMap(result => result.stale.map(item => `${result.versionLabel}: ${item}`));
if (stale.length) {
  console.error(`Generated protocol artifacts are stale:\n${stale.map(item => `- ${item}`).join("\n")}`);
  process.exitCode = 1;
} else {
  for (const result of results) {
    console.log(`${checkOnly ? "Verified" : "Generated"} ${result.count} protocol artifacts for ${result.versionLabel} (v${result.version})`);
  }
}
