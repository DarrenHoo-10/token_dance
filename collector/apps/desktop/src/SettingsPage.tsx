    ...
    <main className="settings-main">
      ...
      <section className="settings-section" aria-labelledby="preferences-heading">
        ...
        <div className="settings-sheet">
          <div className="settings-row"><div><h3>{t("开机启动", "Launch at login")}</h3><p>{autostartNeedsApproval ? t("系统设置尚未允许登录项，请先在系统设置中打开。", "macOS still needs permission for this login item. Allow it in System Settings.") : t("登录电脑后，自动在托盘中运行", "Start quietly in the tray when you sign in")}</p>{autostartNeedsApproval && <button className="settings-inline-link" onClick={() => void perform(openLoginItemsSettings)}>{t("打开系统设置", "Open System Settings")}</button>}</div><Toggle label={t("开机启动", "Launch at login")} checked={(data?.autostart.enabled ?? true) || autostartNeedsApproval} disabled={disabled} onChange={enabled => void perform(async () => { const autostart = await setAutostart(enabled); setData(current => current ? { ...current, autostart } : current); }, true)} /></div>
