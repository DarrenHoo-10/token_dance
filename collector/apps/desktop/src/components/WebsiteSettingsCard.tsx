...
import { DEFAULT_WEBSITE_ORIGIN } from "../website";
...
export function WebsiteSettingsCard({ lang }: { lang: "zh" | "en" }) {
  ...
  return <section className="card" style={{ padding:24, marginBottom:20 }}>
    ...
    <form onSubmit={event => { event.preventDefault(); try { saveWebsiteUrl(url.trim()); setMessage(zh ? "网站地址已保存" : "Website URL saved"); } catch (error) { setMessage(String(error)); } }} style={{ display:"flex", gap:10 }}>
      <input type="url" aria-label={zh ? "网站主页地址" : "Website URL"} placeholder={DEFAULT_WEBSITE_ORIGIN} value={url} onChange={event => setUrl(event.target.value)} style={{ flex:1, minWidth:0, padding:10, borderRadius:8, border:"1px solid var(--border)", background:"var(--bg-base)", color:"var(--text-main)" }} />
