...
impl HttpTransport {
    ...
    /// Fetch the per-device event watermark: statistics days at or before
    /// `ackThroughDay` are already stored server-side and must never be
    /// re-submitted.
    pub async fn fetch_cursor(&self) -> Result<Value, TransportError> {
        const PATH: &str = "/v1/telemetry/cursor";
