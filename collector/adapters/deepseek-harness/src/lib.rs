...
pub const ADAPTER_ID: &str = "dev.tokenshow.adapter.deepseek-harness";
pub const OTLP_SOURCE_ID: &str = "deepseek-harness-otlp";
pub const HISTORY_SOURCE_ID: &str = "deepseek-harness-history";
...
pub fn load_manifest() -> AdapterManifest {
    serde_json::from_str(MANIFEST_JSON).expect("DeepSeek Harness manifest fixture")
}
            ...
            capabilities: self
                ...
                .map(|capability| {
                    ...
                    CapabilityStatus {
                        ...
                        accuracy: enabled.then_some(if capability == Capability::Code {
                            Accuracy::Correlated
                        } else {
                            Accuracy::Exact
                        }),
                        safe_reason_code: (!enabled)
                            .then(|| "DSH_VERSION_OR_SOURCE_UNVERIFIED".into()),
                    }
                })
                .collect(),
        }
...
impl AgentAdapter for DeepSeekHarnessAdapter {
    ...
    async fn probe(&self, _ctx: ProbeContext) -> Result<ProbeReport, AdapterError> {
        ...
        Ok(ProbeReport {
            ...
            detail: if self.detected && !supported {
                Some("unknown DeepSeek Harness version; OTLP disabled and verified history schema only".into())
            } else {
                None
            },
        })
    ...
    async fn setup_plan(&self, _ctx: SetupContext) -> Result<SetupPlan, AdapterError> {
        Ok(SetupPlan {
            plan_id: "setup-deepseek-harness-otlp-v1".into(),
            adapter_id: self.manifest.id.clone(),
            summary:
                "Enable local DeepSeek Harness OTLP without prompt, tool-detail, or content logging"
                    .into(),
            mutations: vec![ConfigMutation::JsonMergePatch {
                path_template: "${USER_HOME}/.deepseek-harness/composition.json".into(),
                patch: serde_json::json!({
                    "env": {
                        "DSH_SESSION_TELEMETRY_ENABLED": "1",
                        "OTEL_EXPORTER_OTLP_ENDPOINT": "http://127.0.0.1:4318",
                        "DSH_OTEL_LOG_PROMPTS": "0",
                        "DSH_OTEL_LOG_TOOL_ARGUMENTS": "0",
                        "DSH_OTEL_LOG_CONTENT": "0"
                    }
                }),
            }],
            required_permissions: vec![],
            verify: vec![VerifyStep {
                id: "deepseek-harness-otlp-safe".into(),
                summary: "OTLP is loopback and content logging remains disabled".into(),
            }],
            rollback: vec![RollbackStep {
                id: "restore-deepseek-harness-settings".into(),
                summary: "Restore the previous DeepSeek Harness settings".into(),
            }],
        })
    }
    ...
    async fn discover_sources(&self, _ctx: SourceContext) -> Result<Vec<SourceSpec>, AdapterError> {
        ...
        sources.push(SourceSpec::JsonlTail {
            ...
            path_template: "${USER_HOME}/.deepseek-harness/sessions/**".into(),
        });
        Ok(sources)
    }
    ...
    async fn health(&self) -> AdapterHealth {
        if self.detected && !self.version_supported() {
            AdapterHealth::Degraded {
                reason: "unknown DeepSeek Harness version; history fallback only".into(),
            }
        } else {
...
fn validate_frame(frame: &RawFrame) -> Result<(), AdapterError> {
    ...
    if valid {
        ...
        Err(AdapterError::decode_failed(
            "frame source kind/id does not match DeepSeek Harness manifest source",
        ))
    }
}
...
fn decode_record(
    ...
    let source_identity = semantic_id
        .map(|_| "deepseek-harness-semantic-event")
...
fn normalize_record(value: Value, source_kind: SourceKind) -> Result<Option<Value>, AdapterError> {
    ...
    let normalized_type = match source_kind {
        SourceKind::Otlp => match name {
            "telemetry.session.started" => "session_started",
            "telemetry.turn.completed" => "turn_completed",
            "telemetry.model.usage" => "model_usage_recorded",
            "telemetry.tool.result" | "telemetry.tool.completed" | "telemetry.tool.failed" => {
                "tool_invoked"
            }
            "telemetry.skill.execution.completed" | "telemetry.skill.execution.failed" => {
                "skill_invoked"
            }
            "telemetry.code.changed" => "code_changed",
            "telemetry.agent.spawned" => "agent_spawned",
            "telemetry.tool.started"
            | "telemetry.tool.invoked"
            | "telemetry.skill.invoked"
            | "telemetry.skill.injected"
            | "telemetry.skill.loaded"
            | "telemetry.skill.execution.started" => return Ok(None),
            _ => return Ok(None),
        },
        SourceKind::JsonlTail => match name {
            "session/start" | "session/started" => "session_started",
            "turn/end" | "turn/completed" => "turn_completed",
            "model/usage" => "model_usage_recorded",
            "tool/result" | "tool/completed" | "tool/failed" => "tool_invoked",
            "skill/execution/completed" | "skill/execution/failed" => "skill_invoked",
            "file/changed" => "code_changed",
            "agent/spawned" | "session/child" => "agent_spawned",
            "tool/call"
            | "tool/started"
            | "skill/invoked"
            | "skill/injected"
            | "skill/loaded"
            | "skill/execution/started" => return Ok(None),
            _ => return Ok(None),
        },
        _ => return Ok(None),
    };
...
fn build_normalized(source: &Map<String, Value>, name: &str, normalized_type: &str) -> Value {
    ...
    if normalized_type == "skill_invoked"
        && (name.ends_with(".failed") || name.ends_with("/failed"))
    {
