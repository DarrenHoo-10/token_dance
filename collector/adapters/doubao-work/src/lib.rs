...
pub const ADAPTER_ID: &str = "dev.tokenshow.adapter.doubao-work";
pub const HISTORY_SOURCE_ID: &str = "doubao-work-sessions";
...
pub fn load_manifest() -> AdapterManifest {
    serde_json::from_str(MANIFEST_JSON).expect("Doubao Work manifest")
}

pub struct DoubaoWorkAdapter {
        ...
        CapabilityReport {
            ...
            capabilities: self
                ...
                .map(|capability| CapabilityStatus {
                    ...
                    safe_reason_code: (!available).then(|| "DOUBAO_WORK_NOT_DETECTED".into()),
                })
                .collect(),
        }
    }
...
#[async_trait]
impl AgentAdapter for DoubaoWorkAdapter {
    ...
    async fn setup_plan(&self, _ctx: SetupContext) -> Result<SetupPlan, AdapterError> {
        Ok(SetupPlan {
            plan_id: "doubao-work-readonly-v1".into(),
            adapter_id: self.manifest.id.clone(),
            summary: "Read Doubao Work local session JSONL without uploading prompts".into(),
            mutations: vec![],
    ...
    async fn decode(&self, frame: RawFrame) -> Result<Vec<NormalizedEvent>, AdapterError> {
        if frame.source_kind != SourceKind::JsonlTail || frame.source_id != HISTORY_SOURCE_ID {
            return Err(AdapterError::decode_failed("unsupported Doubao Work source"));
        }
    ...
    async fn health(&self) -> AdapterHealth {
        if self.detected {
            ...
            AdapterHealth::Degraded {
                reason: "Doubao Work is not installed on this device".into(),
            }
        }
    }
}
...
fn decode_record(
    ...
    let provider = string(object, "provider").unwrap_or_else(|| "doubao".into());
