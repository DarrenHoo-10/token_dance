...
export function HomeLeaderboard({ entries, ownEntry, window: period }: {
  ...
  return <>
    <div className="panel-header">
      ...
      <label className="sky-board-search"><Search size={16} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder={zh ? '搜索开发者' : 'Find a developer'} aria-label={zh ? '搜索当前榜单开发者' : 'Search developers in this board'} />{query && <button type="button" onClick={() => setQuery('')} aria-label={zh ? '清除搜索' : 'Clear search'}><X size={14} /></button>}</label>
    </div>
    {visible.length ? <LeaderboardTable entries={visible} ownEntry={term ? null : ownEntry} /> : <p className="leaderboard-empty">{zh ? (term ? '当前榜单没有匹配的开发者' : '暂无账号') : (term ? 'No matching developers in this board' : 'No accounts yet')}</p>}
