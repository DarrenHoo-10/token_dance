...
export function getArticles(zh: boolean): DocArticle[] {
  return zh ? [
    ...
    { slug: 'sources', label: '支持的工具', group: '了解 TokenDance', title: '支持的工具', lead: '采集用量与查询套餐额度是不同能力。能显示 Token，不代表同时支持额度、费用或全部历史数据。', sections: [
      { id: 'tools', title: '主要来源', columns: ['工具', '使用说明'], rows: [ ['Codex', '从本地会话记录读取用量；有近期额度记录时展示额度。'], ['Claude Code', '查看采集到的 Token；没有对应能力时不显示为零。'], ['Cursor', '用量与套餐额度独立展示，额度需有效的本机登录状态。'], ['Grok Build', '从本地已完成轮次日志采集用量；周额度为 Grok 产品共享额度。'], ['ZCode', '额度查询支持个人 Coding Plan；其他套餐不据此推算。'], ['OpenCode', '从本机 opencode.db 读取已验证 schema 的会话 Token、AI 交互次数和 edit/write 变更行数。'], ['WorkBuddy / Doubao Work', '检测到本机安装后采集本地会话 JSONL；没有会话文件时不把用量显示为零。'], ['Pi / DeepSeek Harness', '支持范围和可用指标以客户端来源状态与实际记录为准。'] ] },
    ...
    { slug: 'sync', label: '采集与同步', group: '了解 TokenDance', title: '采集与同步', lead: '本机面板记录设备上的用量，网站汇总已成功同步的记录。网络、账号和统计时区都会影响两边的展示。', sections: [
      ...
      { id: 'time', title: '统计时间与历史', paragraphs: ['网站与桌面的统计日都按北京时间（UTC+8）划分。「全部时间」指仍有记录的全部历史，不保证能恢复升级前已清理的数据。', '不要将未知或不支持的数据当成零，也不要将未同步的本机记录当成网站已收录数据。'] },
    ] },
    ...
    { slug: 'faq', label: '常见问题', group: '帮助与更新', title: '常见问题', lead: '先确认账号、采集状态和统计周期，通常就能定位用量显示问题。', sections: [
      { id: 'no-ranking', title: '客户端有数据，排行榜为什么还没有？', paragraphs: ['确认客户端和网站登录同一账号、采集没有暂停，并等待自动同步。检查同步提示；若登录失效需重新登录。统计周期按北京时间划分，请确认所选窗口。'] },
    ...
    { slug: 'sources', label: 'Supported tools', group: 'About TokenDance', title: 'Supported tools', lead: 'Token collection and plan quota lookup are separate capabilities. Token usage does not imply support for quotas, costs or all historical records.', sections: [
      { id: 'tools', title: 'Main sources', columns: ['Tool', 'What to expect'], rows: [ ['Codex', 'Reads local session usage; quotas appear when recent quota records exist.'], ['Claude Code', 'Shows collected tokens. Unsupported capabilities are not shown as zero.'], ['Cursor', 'Usage and plan quotas are separate. Quotas require a valid local sign-in.'], ['Grok Build', 'Reads completed local turns. The weekly quota is shared across Grok products.'], ['ZCode', 'Quota lookup supports personal Coding Plan; other plans are not inferred.'], ['OpenCode', 'Reads tokens, AI turns and edit/write line changes from a fingerprint-verified local opencode.db.'], ['WorkBuddy / Doubao Work', 'Detected installs collect local session JSONL. Missing session files are not shown as zero usage.'], ['Pi / DeepSeek Harness', 'Available metrics depend on the source status and records shown in the app.'] ] },
    ...
    { slug: 'sync', label: 'Collection & sync', group: 'About TokenDance', title: 'Collection & sync', lead: 'The desktop shows local device usage. The website aggregates records that have synced successfully. Network, account state and time zones can affect the totals.', sections: [
      ...
      { id: 'time', title: 'Time windows and history', paragraphs: ['Website and desktop statistics days both use Beijing time (UTC+8). All time covers history still recorded, with no promise to recover data cleared before an upgrade.', 'Unknown or unsupported values are not zero. Local records that are pending sync are not yet included in website totals.'] },
    ] },
    ...
    { slug: 'faq', label: 'FAQ', group: 'Help & updates', title: 'Frequently asked questions', lead: 'Start by checking your account, collection status and reporting period.', sections: [
      { id: 'no-ranking', title: 'Why is local usage missing from the board?', paragraphs: ['Use the same account on desktop and web, resume collection and wait for automatic sync. Check sync errors and sign in again if needed. Periods are cut on Beijing time.'] },
