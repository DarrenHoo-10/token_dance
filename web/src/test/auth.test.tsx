...
describe('Auth & Onboarding Flow Tests', () => {
  ...
  it('renders OnboardingPage with always-public usage', async () => {
    vi.spyOn(api, 'getSession').mockResolvedValue({
      authenticated: true,
      user: {
        handle: null,
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText('资料默认公开').length).toBeGreaterThan(0);
      expect(screen.getByText(/用量趋势、详细资料页和排行榜默认对社区可见/)).toBeInTheDocument();
