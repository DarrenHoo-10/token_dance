...
vi.mock('@/api/client',()=>({api:{getLeaderboard:vi.fn(),getPersonalSummary:vi.fn()}}));
...
describe('personal leaderboard footer',()=>{
  beforeEach(()=>{
    ...
    vi.mocked(api.getLeaderboard).mockResolvedValue({entries:[{...own,rankNo:1,handle:'leader',displayName:'Leader'}],totalEntries:12486,nextCursor:'20'} as never);
    vi.mocked(api.getPersonalSummary).mockResolvedValue({ranking:{entry:own}} as never);
  });
  ...
  it('does not duplicate users inside the top 1000 or fetch private data when signed out',async()=>{
    vi.mocked(api.getPersonalSummary).mockResolvedValue({ranking:{entry:{...own,rankNo:1000}}} as never);
    const first=page();await screen.findByText('12,486');
    expect(screen.queryByRole('row',{name:'我的排名'})).not.toBeInTheDocument();
    first.unmount();vi.clearAllMocks();auth.authenticated=false;page();await screen.findByText('12,486');
    expect(api.getPersonalSummary).not.toHaveBeenCalled();
  });
  it('clears the previous period personal row when the next period has no rank',async()=>{
    ...
    vi.mocked(api.getPersonalSummary).mockResolvedValue({ranking:{entry:null}} as never);
    fireEvent.click(screen.getByRole('tab',{name:'近 7 天'}));
    await waitFor(()=>expect(api.getPersonalSummary).toHaveBeenLastCalledWith('7d'));
    expect(screen.queryByRole('row',{name:'我的排名'})).not.toBeInTheDocument();
  });
});
