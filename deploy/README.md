...
**生产环境和桌面端正式发布仅使用 `main`。** 改动先合入 `origin/main`，再从干净的 `main` 工作区构建；构建前拉取远端并确认 `HEAD` 与 `origin/main` 一致。

**测试服务使用独立 `release` 分支**，从干净工作区构建，允许发布已经通过 CR 和功能测试、尚未合入 main 的改动。每次发布记录实际分支及完整提交 SHA。测试服务配置与操作见 [测试服务发布](test/README.md)。以下路径与操作适用于生产环境。
