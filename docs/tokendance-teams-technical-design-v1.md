...
日期：2026-09-06。状态：开发设计稿。代码勘察基线（初稿）：`e0103e3`。**2026-09-12 修订成员关系 / 自动共享 / 退出留量**，与 [静态物化方案](tokendance-teams-static-metrics-v1.md) 对齐；不再把 `team_memberships.ended_at` 当产品台账，也不再用 `team_sharing_grants` 做分析资格。

本次修订将可直接分享、多人使用的成员邀请链接纳入首版，与定向邮箱邀请并列；两种方式都经过本人确认和单团队事务。

产品依据：[团队功能与交互方案](tokendance-teams-product-design-v1.md)。引用的现有文件是本次实际检查过的接入位置。

## 1. 确定的边界与核心决策

1. **每个账号同时最多属于一个团队。** 创建、接受邀请共用一个数据库名额；所有者、管理员、成员均计入。名额锁是 `user_current_teams` PK(`user_id`)。退出后可重新加入，因为 occupancy 已删除——**不把 ended `team_memberships` 行当作产品记录**，leave/remove 删除当前成员行。
...
4. 公开隐私与团队用量完全独立；不使用 `users.leaderboard_visibility='team'` 作为团队鉴权依据。加入不公开个人资料，但加入即计入全部用量维度。
5. **在队即自动共享** Token、具名贡献、classification、cost。资格 = 当前 occupancy + `occurred_at >= joined_at` + 账号 active + 团队 active。不回填加入前个人历史。`team_sharing_grants` 退出分析读路径。
6. 团队分析使用独立、可失效的统计快照（切读后为静态日表上的稳定句柄）。单次响应返回同一版本的指标、趋势与构成。
7. 同时支持定向邮箱邀请与可分享邀请链接。邮箱邀请要求验证邮箱匹配；分享链接持有者登录后可确认加入为 member，不绑定指定邮箱。打开 / 预览不写成员关系；加入不收集用量开关。链接「已用」记在 `team_invite_link_joins` PK(`link_id`,`user_id`)，不是成员历史。
8. 团队导出通过 API 鉴权下载，不向浏览器返回对象存储直链。

首期实现创建页、邀请与加入、总览、成员管理、Agent / 模型分析、资料与头像、导出、审计、退出与解散。不实现用量共享设置页。预算、公开团队榜、自定义角色、项目层级、SSO 不纳入本方案；多团队不是后续开关。
...
| `usage_events` | 存在 `occurred_at`、`received_at`、Token 分项、费用来源，索引 `(user_id, occurred_at, event_pk)` | 旧协议经个人日汇总进入团队；资格按 occupancy + `joined_at`，不再按 grant 窗切片 |
...
- Service：角色决策、字段规范化、时间范围、结果 DTO；数据库状态相关检查必须在事务内复核。用量资格不再做 grant 差量计算。
...
- 时间使用 UTC `DATETIME(3)`；有效时间均为左闭右开 `[from, to)`。`joined_at` 在事务获得业务锁后读取数据库当前时间一次，不能使用客户端时间或排队前时间。
...
| `team_memberships` | membership_id、team_id、user_id、base_role、sharing_version、joined_at、ended_at、end_reason | PK(membership_id)；UNIQUE(team_id,user_id,membership_id) 供组合 FK；**产品上只保留当前 stint**。leave/remove **DELETE** 该行，不写 `ended_at`。`ended_at`/`end_reason` 列为 0005 遗留，停止作为产品功能。重新加入创建新 ID |
| `user_current_teams` | user_id、team_id、membership_id、joined_at | **PK(user_id)**；UNIQUE(membership_id)；组合 FK(team_id,user_id,membership_id) → memberships；索引(team_id,user_id)。与 memberships 1:1 |
| `team_sharing_grants` | grant_id、membership_id、dimension、starts_at、ends_at、revoked_at、active_dimension | **分析资格退役**。0005 已落地，不在本方案 DROP。静态读/写路径不使用。过渡期 occupy 可写四维全开以便旧扫描；随后停写 |
...
| `team_invite_link_joins` | link_id、user_id、membership_id、joined_at | PK(link_id,user_id) 为消费键；退出不删除该行。静态方案 0015 **DROP** 指向 memberships 的 FK，以便 DELETE 当前成员行；`membership_id` 可成悬空引用，不是成员台账 |
...
团队创建同时插入 `team_source_revisions`，source_revision 初值 0。`teams.auth_revision` 初值 1。资料单独使用 profile_version。`sharing_version` 列为 0005 遗留，用量资格不再递增它。
...
迁移中增加明确的 CHECK：active 团队 owner 非空；0005 成员 ended_at/end_reason 成对存在（实现停止写入 ended 行，leave 改为 DELETE）；grant 表约束保留但不驱动分析；pending 邀请 active_recipient_hash 非空且等于 lookup hash，终态必须为空；快照活动 key 与状态一致。涉及可空字段时显式使用 IS NULL / IS NOT NULL，不能依赖 SQL UNKNOWN 自动拒绝。
...
业务锁顺序：已需要的 installation（现有 Ingest / 设备删除）→ 相关 users 按 ID 排序 → teams 按 ID 排序 → current / membership / invitation / invite_link → link_joins → source revision / jobs / audit / receipts。不得先拿 teams 再等待用户锁。leave 在已持有上述锁后 MERGE 静态行。
...
BEGIN
  ...
  忽略请求 sharing flags；在队即全维度。过渡期若仍写 grants，插入四维全开且 starts_at=T
  入队该成员从 joined_at 起的静态 stint 回填（见静态物化方案）
...
- 当前名额属于同团队且邀请仍有效：将邀请标记 accepted、关联现有 membership、释放 active_recipient_hash 并记录回执，返回 alreadyMember；不增加成员，不覆盖现有角色。
- 邀请已被同一用户接受：仅在其仍属于 accepted_membership_id（且 occupancy 仍是该 ID）时返回已有结果；已退出则 410，禁止旧邀请复活。
- 正常接受：写当前 membership + occupancy（忽略 sharing flags），标记 accepted、记录 accepted_membership_id，释放 active_recipient_hash，增加 auth_revision、写审计 / 幂等回执，同一事务提交。入队从 `joined_at` 起的静态回填。

不再根据 ended `end_reason=removed` 拒绝邮箱邀请。移除事务仍然撤销该用户当前验证邮箱匹配的本团队 pending 邀请。重入队需要**新**的定向邮件或新链接。
...
| 管理员移除 | actor 为 admin，目标有效且 base_role=member，目标不是 owner | 同一事务：具名静态行 MERGE 进历史用户桶，删队列，DELETE occupancy 与当前 membership（不写 ended_at），撤销目标发出的 pending 邀请 / 链接；auth_revision++ |
| 所有者移除 | 目标有效且不是自己 | 同上 |
| 主动退出 | 当前用户不是 owner | 同上；提交后名额释放。已计入用量留在历史用户桶 |
| 转移所有权 | actor 是 owner，目标为有效、active 账号成员；expectedAuthRevision 匹配 | 切换唯一 owner 指针，旧 owner 基础角色改 admin；双方都仍属于原团队 |
| 解散 | actor 是 owner；团队名确认与版本匹配 | team=dissolved；**不 MERGE**；DELETE 该队全部静态行（含历史桶）与队列；DELETE 全部 occupancy+membership；撤销邀请 / 导出；auth_revision++ |
...
3. 当前属于其他团队返回 409 TEAM_MEMBERSHIP_EXISTS。当前已在本团队则返回 alreadyMember，不改变角色、不插入消费记录、不扣次数。
4. **删除** `end_reason=removed` → 403 `TEAM_REINVITATION_REQUIRED` 分支。无成员历史台账后无法、也不再根据「上次被移除」拦截。
5. 对本链接已有消费记录（`team_invite_link_joins` PK）返回 410 `TEAM_INVITE_LINK_ALREADY_USED`，即使当前 membership 行已 DELETE。否则校验 used_count<max_uses，并在同一事务创建当前 membership + occupancy（忽略 sharing flags）、登记 link_joins、used_count++、auth_revision++、审计与幂等回执，入队从 `joined_at` 起的静态回填。为该用户仍在运行 / 未核对完成的数据删除补登记屏障。
6. 提交后返回 200 TeamContext。任意错误回滚成员、消费计数和审计，禁止先扣次数再调用另一个独立入队事务。
...
创建者退出、被移除、注销、暂停，或从 owner/admin 降为 member，必须使其活动链接失效；仅从 owner 转为 admin 仍保留 member 邀请权限。解散撤销全队链接。撤销只影响未来加入，不移除既有成员，也不回滚他们已计入的用量。

## 6. 用量资格与时间语义

### 6.1 在队即全维度（无个人开关）

2026-09-12 锁定：加入团队即自动计入 base Token、具名贡献、classification、cost。没有「只加入不共享」，没有 PATCH 用量开关。`loadTeamAnalysisGrants` / `grantCoversTime` / `analysisVisibilityMask` 退出静态资格路径。当前成员写出恒定掩码 `visNamed|visClassification|visCost`（=7）。

`team_sharing_grants` 表保留（0005 已落地），不在本方案 DROP。分析读/写停止使用它。过渡期 `occupyMembership` 若仍插入 grants，必须忽略客户端 flags、四维全开、`starts_at=joined_at`，仅服务于未切读的旧 `ProcessTeamAnalysis`。`UpdateSharingTx` / `applySharing` 不再改变资格。

公开资料 / 头像与用量无关，继续独立。

加入时刻是唯一时间闸门。下列旧时间线**作废**：

```text
作废：09:00 入队未共享 → 10:00 开 base → 才开始计入
现行：09:00 入队 → occurred_at >= 09:00 的用量全部计入，含分类与费用
```
...
```text
team.status = active
AND user_current_teams 持有该 (user_id, team_id, membership_id)
AND users.account_status = 'active'          -- 写出时；暂停见静态方案 K14
AND e.user_id = membership.user_id
AND e.occurred_at >= membership.joined_at
visibility_mask = 7
```

- 延迟上传按 `occurred_at` 判断。加入前的个人历史即使此刻在队也不计入。
- 加入时对**已存**且 `occurred_at >= joined_at` 的事实做有界回填，否则 join 点击前数分钟已 Accepted、当时无 occupancy 的事件会丢。这不是回填加入前数年历史。
- 离开之后到达的上传，即使事件落在旧任期，也不得写入具名行或历史用户桶。
- 浏览器和 Collector 不提供可信 teamId。服务器只看 occupancy 与事件归属。
- 来源没提供名称使用 `unknown` 桶。不再因成员关分类而使用 `unshared_classification`。
- 退出 / 移除：具名行 SUM 进该队匿名历史用户桶（按日、按 agent/model/cost），然后 DELETE 具名行与当前 membership。历史用户不排名、不出现在成员名单。
- 聚合也可能让小团队成员推断彼此规模，因此不承诺数学意义的匿名；历史用户桶保证不保留「属于哪一个前成员」。

## 7. 团队分析与统计快照

### 7.1 为什么单独做快照

个人 UTC 日聚合已丢失事件时刻，不能把加入日整天加进团队。v2 按 `occurred_at >= joined_at` 过滤；旧汇总仅当该 UTC 日 00:00 UTC ≥ `joined_at` 才纳入。团队分析最终只向 Web 返回聚合数据。切读后读源为静态日表，见静态物化方案。
...
2. **切读前**：查询先检查当前团队访问和 auth_revision，再找日期范围一致、rule_version 一致的 ready 快照；30 秒内沿用。普通新增数据可用旧快照并标记 refreshing；auth_revision 不同则绝不能回退旧值。**切读后**：不再走这条 30 秒快照查找，也不调用 `GetOrQueueAnalysis`。GET UPSERT 规则 6 句柄（48h TTL）并读 `team_member_day_metrics`，见 [静态物化方案](tokendance-teams-static-metrics-v1.md) §5.9 / §5.12。
3. **切读后（规则 6 / 静态日表）**：无删除屏障时 GET 恒 200 `state=ready`。空就是 empty-state（「等待首次同步」/「此范围没有数据」），不是数字 0，也不是 202 骨架。202 `updating` **仅**删除屏障。切读前的 queued 扫描路径在 PR6 退役；实现与验收以 [静态物化方案](tokendance-teams-static-metrics-v1.md) K7/§5.9 为准。
4. Worker 以 `FOR UPDATE SKIP LOCKED` 领取任务，赋新 lease_token 和递增 lease_generation，领取事务立即提交。
5. 另开 REPEATABLE READ 一致性读事务；在同一快照读取团队 auth_revision、source_revision、当前 occupancy 和范围内事实。不能先读一个 MAX(event_pk) 再在多个 READ COMMITTED 查询中拼出混合状态。切读后 GET 不再走这条扫描。
6. 在服务端按团队时区归日、全掩码与维度聚合；使用独立连接分批写本次 snapshot + build_generation 的 rows，构建中行不被 API 读取。租约接管使用新 generation，旧 Worker 的迟到写入只能落入旧代次，不能覆盖新代次行。
...
8. 构建期间 source_revision 增长不必丢弃已有结果：它是 as_of 时刻的一致快照，正常发布并触发后续刷新；auth_revision 改变必须丢弃。读取时选同 auth_revision 下 source_revision / as_of 最新者，旧 Worker 不能覆盖新快照。
...
- 活跃成员：周期内至少有一条符合条件的有效 model_usage 事件，且属于**当前**成员；分母为读取时当前成员数。不再单独统计「共享成员」。
- 贡献榜只排序当前成员的 Token。加入即具名计入。总量 / 趋势 / 分布包含历史用户桶，故贡献榜之和可以小于团队总量；不能强制重标到 100%。并列使用 DENSE_RANK，稳定次序按 userId。历史用户不出现在榜上。
- Agent / 模型筛选作用于分类行；选全部时含 `unknown`。所有指标、分布、成员周期数据及导出绑定同一个 snapshotId / filtersHash。
...
覆盖率细化为：可可靠关联已记录费用的有效 model_usage 事件数 / 全部有效 model_usage 事件数；估算覆盖率单独返回。独立 cost_recorded 不充当额外分母。只有费用而无可关联 usage 时，金额可标记“来源记录”，覆盖率为 null，并展示 unattributedCostCount。
...
物化时先产生每个有效 model_usage 事件唯一的一条用量贡献，再关联可证明归属的费用；费用币种展开不能重复累计 Token 或 usage 分母。只有独立费用的贡献行，其 Token / usage 计数为 0。金额可以精确关联到单条 usage 时才进入对应分类和成员细分；不能拆分的跨分类费用留在“未分配费用”，不按 Token 占比伪造分摊。已记录覆盖与估算覆盖对 usage ID 去重，二者不重叠。费用事件同样要求 `occurred_at >= joined_at` 且当时仍在队。

## 8. HTTP API 契约

### 8.1 通用约定

- 下表均相对于 `/api/v1`，全部要求 Session。写操作还要求 X-CSRF-Token。客户端不得指定 actorUserId、ownerUserId、joinedAt。
...
| GET `/teams/{teamId}/members/{membershipId}` | 该当前成员的周期详情 | 有效成员；当前 occupancy 即可打开；snapshotId 必填 |
...
| POST `/team-invitations/{invitationId}/accept` | 200 TeamContext 与 alreadyMember 标记 | 邮箱匹配；expectedInvitationVersion、幂等。body.sharing 若存在则忽略 |
...
| POST `/team-invite-links/{linkId}/accept` | 200 TeamContext 与 alreadyMember | active 已验证账号；body.token、expectedLinkVersion、幂等。body.sharing 若存在则忽略 |
| GET `/teams/{teamId}/my-sharing` | 过渡期可返回隐含全开状态 | 有效成员；**不是**用量授权面，Web 不再展示 |
| PATCH `/teams/{teamId}/my-sharing` | 忽略 flags，不改变资格 | 过渡期 no-op；随后可 404 |
| GET `/teams/{teamId}/analysis` | 200 `ready`（空为空）；**仅删除屏障** 202 | 有效成员；日期 / Agent / provider / model / snapshotId。切读后 UPSERT 48h 句柄，不排队范围扫描 |
...
analysis 首次返回每个构成列表的前 20 条及 nextCursor；同接口携带 snapshotId、collection=agents/models/contributions、cursor 获取对应列表后续页，保持同一范围与筛选。趋势最多 90 个日点，无需分页。分类 ID 以 provider + model 等稳定组合区分，不能只按显示名合并；未知分类使用保留的桶类型，不伪造真实 model ID。历史用户不是分类桶。
...
`sharing` 可省略。若客户端仍发送该对象，**忽略**，不作为计入条件，不返回 400。成功响应：
...
PATCH my-sharing **不再改变用量资格**。过渡期可 200 返回隐含全开状态且不 bump 分析资格；Web 删除该入口。后续 PR 可改为 404。

### 8.4 分析响应

查询例：`GET /teams/{teamId}/analysis?range=7d&agent=codex`。查询参数不接受 userId；不能通过筛选推断已退出成员的身份。
...
示例的 trend 数组与 agents / models / contributions.items 省略了实际行；正式响应必须与 summary 同源。用量值采用 value + state，状态为 available / empty / unsupported；null 与 0 不互换。费用列表为空表示无该类可加总费用，来源缺失由 quality 说明；费用分母为 0 时比例=null。`not_shared` 不再作为当前成员的用量状态。range.toExclusive 保留所选完整日期边界，dataToExclusive 是本次快照实际截断时刻。

切读后不再因「没有范围快照」返回 202。**仅删除屏障**返回 202：
...
`currentSharingMembers` 为兼容字段，**恒等于** `currentMembers`。leave 导致的新 `authRevision`：200 + 保留上一帧直到新 DTO，不得整页清数据。409 `TEAM_SNAPSHOT_OBSOLETE` 只用于仍拿旧 `snapshotId` 拉导出/详情的请求。

### 8.5 错误码

| HTTP / code | 条件 | 前端行为 |
| --- | --- | --- |
| 400 API_INVALID_ARGUMENT | 名称、时区、日期不合法 | details.fieldErrors 定位输入，保留表单 |
...
| 409 TEAM_VERSION_CONFLICT | 角色 / 资料版本过期 | 保留草稿并刷新当前状态 |
...
| 409 TEAM_SNAPSHOT_OBSOLETE | 请求旧 auth_revision 快照 | 清旧值，重新请求 analysis |
...
| 410 TEAM_INVITE_LINK_ALREADY_USED | 本账号已经消费过此链接（`team_invite_link_joins`），无论当前是否仍在队 | 说明需新的链接或新的邮件邀请；不重复扣次数 |
...
预览只返回 token 授权查看的团队名、邀请者展示名、固定角色、到期时间、linkVersion、effectiveState，以及当前账号是否已在队 / 此链接是否已使用；不返回成员名单、成员邮箱、用量、私有头像或内部对象地址。不根据「上次被移除」返回单独状态。次数上限及管理详情只向管理者返回。错误 token 无论 ID 是否存在均为相同 404；正确 token 的到期 / 撤销 / 用尽可以返回相应 410。
...
- 导出类型为 daily / agents / models / members；成员导出含当前成员具名行，外加至多一行匿名「历史用户」小计，不输出前成员 membershipId。来源无名分类可汇总到 unknown 桶。
...
覆盖 create、invite、resend、revoke、accept、invite_link_create / copy / revoke / regenerate / accept、role_change、remove、leave、transfer、dissolve、export_create / download / revoke、avatar_change。管理动作与审计同事务提交；审计写失败则管理事务失败。invite_link_copy 记录服务端发放 shareUrl 的动作，不声称已获知客户端剪贴板实际写入结果。不再把 sharing_change 当作用量授权审计的产品面。

safe_details 只记录角色前后值、资源 ID、版本、结果码，不记录 Prompt、设备路径、原始邮箱、密钥、下载地址或完整请求体。默认团队管理审计保留 180 天；账号注销时对 actor / target 的身份引用与可能含个人内容的 details 去标识，保留无 PII 的动作类型和时间。

## 11. 撤权、删除与生命周期

### 11.1 失效的事务边界

所有成员进入 / 离开、角色变化、影响该团队的数据删除都增加 teams.auth_revision；新事件和补价只增加 source_revision。用量开关不再存在，因而不会因 PATCH my-sharing 单独 bump。把角色变化也纳入 auth_revision 是首版保守策略。
...
- 普通成员 / 管理员：在 RequestDeletionTx 设置 deletion_pending 的同一事务里**先按 leave MERGE 进历史用户桶**，再 DELETE occupancy 与当前 membership，撤销其发出的有效邮箱邀请和分享链接并增加团队 authRevision。合并后该量已匿名，后续个人删除不得从历史桶扣减。
- 取消注销只恢复账号，不自动恢复团队关系或已撤销邀请；用户需重新受邀。
- Worker 在 deleting_objects 阶段处理该用户创建的团队导出对象及失效头像上传意图；在 deleting_identity / reconciling 阶段去标识邀请邮箱、链接消费身份、审计引用，保持既有删除代次校验。不依赖 ended membership 行。
- 解散后的团队 owner 引用在其账号匿名化时清空；active 团队不能出现无 owner 状态。不能保留可还原的邮箱副本。
...
用户可能在删除过程中退出或加入另一团队。离开走 MERGE 历史用户，不把旧贡献从总量抹掉。创建 / 接受邀请事务在用户锁下检查该用户尚未核对完成的删除请求，为新团队补登记屏障。删除完成事务同样先锁 user，再锁其全部屏障 team，防止漏掉并发加入。新团队可以正常创建或加入，但在有关删除完成前不发布分析。解除关联后才允许清理原 deletion request，避免外键悬挂或错误提前解除。对**当前成员**的事实删除只重建该人具名行；已合并的历史用户桶不按人回滚。

账号 suspended 时现有 RequireAuth 阻断其访问；同事务作废其创建的活动分享链接，authRevision++。**暂停仍是当前成员，具名统计保留，不 MERGE 进历史用户**，直到 leave/remove。暂停不释放其团队名额，也不自动转移所有权。

### 11.4 后台清理

- **切读后**规则 6 句柄 GET 每次续期 `expires_at=now+48h`（≥ 导出 24h）。`expireTeamSnapshots` 仅当 `expires_at <= now` 且无导出引用（queued/running，或 completed 且 job.expires_at 未到）时标 expired。GET UPSERT 可把 expired 行复活为同一 `snapshot_id` 的 ready。切读前规则 5 扫描快照仍为 30 分钟 TTL，PR6 退役。成员离开立即从 occupancy 消失；历史用户桶留量，不是「授权撤销掉总量」。
...
- 解散团队清除 current、当前 membership、该队静态行（含历史用户桶）、快照、导出、头像；审计按去标识保留策略处理，不删除成员个人 usage_events / telemetry_events。

## 12. 前端页面与状态管理

### 12.1 路由与组件边界

沿用现有 AppLayout、AuthContext、LocaleContext 和状态组件；新增 TeamContext，只在内存保存当前团队及版本，不持久化团队统计。网站 basename 仍由 `import.meta.env.BASE_URL` 提供，禁止硬编码根站 `/api/v1` 或重复拼接 `/token-dance/`。
...
| `/teams/invitations/:invitationId` | 邮箱绑定的邀请预览、确认加入（无用量开关） | 邀请详情 + me/team |
| `/teams/join/:linkId` | 读取分享凭证、登录回跳、预览、确认加入 | 登录后 POST link preview + me/team；仅点击加入才 POST accept |
...
| `/teams/:teamId/settings` | 资料、头像、操作记录及退出 / 解散 | TeamContext；管理者按需 audit-events。无「我的共享」 |
...
不引入新的全局状态库；首版用现有 React hooks、Context 与 API client。请求函数增加 AbortSignal，离开页面、快速切换日期或 authRevision 变化时中止旧请求；另用 request sequence + teamId + authRevision 检查迟到响应，不能只靠 AbortController。leave 导致的 authRevision 变化不得清空上一帧 ready 分析 DTO。
...
两类邀请在登录后展示团队名、邀请人、角色、有效期，说明加入后自动计入全部用量维度且只从加入时刻起算，由本人点击接受。未登录保留回跳；只有定向邮箱邀请执行收件邮箱匹配，分享链接执行 token 验证。已有其他团队时显示当前团队入口和单团队说明，不提供自动替换团队按钮。

**不实现用量共享设置页。** 设置页只保留资料、头像、退出 / 转移 / 解散与审计。
...
分享链接加入页状态：读取凭证 → 登录 / 建档 → 验证链接 → 待确认 → 提交 → 已加入；旁路状态为凭证丢失、已在本队、已有其他团队、到期、撤销、次数用尽或链接已使用。网络重试复用 Idempotency-Key，成功只导航一次。链接凭证按第 9.4 节暂存和清除，错误上报必须主动过滤 fragment 与 token 字段。

成员详情抽屉约 420 px，小屏占满可用宽度；打开后移动焦点、Esc 关闭、关闭后回到来源行。当前成员均可打开用量详情。团队同步状态只来自该成员已计入事件的 max_received_at，不暴露个人设备在线状态。已退出者不在名单中。
...
| 服务端状态 / 数据 | 页面表达 |
| --- | --- |
| 首次请求、无事实、无屏障 | 200 ready + empty-state「等待首次同步」/「此范围没有数据」；**不是**骨架，**不是**数字 0 |
| 删除屏障 202 updating | 唯一整页骨架 |
| leave 导致新 authRevision | 200；保留上一帧直到新 DTO；贡献榜无该人，总量含历史用户 |
| ready + refreshing，加入切片 in-flight | 横幅 memberBackfill；KPI empty-state 不是数字 0 |
| ready + refreshing，已有数据 | 可显示截至 asOf 的旧值与更新时间，后台刷新 |
| 已加入，但无有效数据 | “等待首次同步”或“此周期暂无数据”，与真实 0 区分 |
| 部分费用 / 分类不可用 | 保留有效用量，费用显示 —、覆盖率或 unknown 分类桶 |
| 范围内含历史用户用量 | 总量 / 趋势 / 分布包含；贡献榜不含；一条脚注，不是排行行 |
...
| `server/internal/teams/metrics.go` | Token 规范化、费用去重与覆盖率；配套 fixture 测试。资格改为 occupancy + joined_at |
| `server/internal/store/store.go` | 新 TeamsStore 子接口，以及用例输入 / 输出约定 |
| `server/internal/store/mysql/teams.go` | 关系、邮箱邀请、角色、幂等回执；`occupyMembership` / `closeMembership` 按静态方案改为无 ended 台账 + leave MERGE |
...
Store 的事务接口以业务命名：CreateTeamTx、CreateInvitationTx、AcceptInvitationTx、CreateInviteLinkTx、RevokeInviteLinkTx、RegenerateInviteLinkTx、AcceptInviteLinkTx、ChangeMemberRoleTx、RemoveMemberTx、LeaveTeamTx、TransferOwnershipTx、DissolveTeamTx、QueueTeamExportTx。`UpdateSharingTx` 退役为过渡 no-op。输入包含 actor、目标、期望版本、规范化请求与幂等信息；输出明确 Changed / Replay / AlreadyMember，不让 Handler 猜 RowsAffected 的业务含义。两种接受方式调用同一内部成员写入步骤，但不能通过调用两个独立 Tx 嵌套来实现。

分析侧切读前分为 ResolveTeamRange、GetOrQueueAnalysis、ClaimAnalysis、BuildAnalysis、PublishAnalysis。切读后 GET 读静态日表，见静态物化方案。资格函数只接收 occupancy + `joined_at`，不能通过某个调用方忘记传 filter 就默认允许加入前数据。
...
这批表初始为空；**不从 leaderboard_visibility=team 推断成员，不自动创建团队**。加入即计入用量，不再「打开授权」。仅对既有 outbox 增加可空字段及索引，已有认证邮件保持可处理。MySQL DDL 会隐式提交，不能把多条建表包在 BEGIN 中就认为可以整体回滚；失败依照现有 Runner 的实际状态和校验恢复，禁止直接清 dirty 标记跳过。

### 14.2 交付顺序与门槛

| 阶段 | 交付内容 | 完成条件 |
| --- | --- | --- |
| A：关系与创建 | 迁移、角色权限、单团队事务、邮箱与分享链接、创建 / 接受页 | 并发唯一性、邮箱绑定、链接防重与最后次数竞争通过；加入即计入 |
| B：数据 | 源版本、快照 / 静态日表、费用口径、全部删除屏障与撤权 hooks | 退出 MERGE 历史用户；租约接管和数据删除不复活离队写入 |
| C：可用页面 | 总览、成员、分析、设置、同步状态与 i18n | 真实接口联调、三种宽度和全部主要错误状态通过 |
| D：管理与交付 | 导出、头像、审计、清理、监控和发布演练 | 端到端与实际 MySQL 检查通过，可开启功能 |

阶段表示依赖拆分。开发环境可用测试账号逐段验收，公开启用必须同时具备退出合并、数据删除与任务发布保护。

### 14.3 灰度与回退

新增服务端功能开关，初始关闭；迁移后先部署包含全部版本 / 撤权 hooks 的 API 和 Worker，再部署 Web，最后对测试账号启用。仅把按钮隐藏不能控制 API，创建与接受入口也要受服务端开关约束。

读取、新建 / 加入、导出可分别停用，但已有成员的退出、所有者处理与账号删除路径必须保持可用；停用分析时返回明确不可用状态，不转读个人表或旧 team 榜单。所有写入 usage 的进程都升级后才能开放团队，否则旧补价 / Ingest 路径可能漏增源版本。

应用回退保留新增表和数据，不执行 DROP。团队一旦有真实成员，不能直接回退到完全不认识团队权限 / 删除 hooks 的旧 API 或 Worker；应部署兼容修复版本，或关闭团队读取及新建并保留安全生命周期服务。
...
- 高基数行采用有界分批聚合与数据库暂存，不能将 90 天所有原始事件装入单个 Go map。Top 列表默认 20 条，另有完整分页；其余贡献用“其他”桶保持总和一致，不丢掉 unknown 分类桶。filter-options 同样分页 / 搜索。
...
- 目标测试规模：100 位成员、范围内 100 万有效事件；非媒体管理接口 p95 < 500 ms，ready 分析读取 p95 < 800 ms，普通构建 30 秒内完成。达不到时先看查询计划、联接和暂存开销，再落到静态日表（见静态物化方案）；不通过把加入前历史灌进团队换性能。
...
| 编号 | 场景 | 核心断言 |
| --- | --- | --- |
| T01 | 同用户并发 create/create、create/accept、accept/accept（不同团队） | 至多 1 条 current、1 个有效身份；失败事务不留孤儿 team / membership / accepted 邀请 |
| T02 | 创建已提交但响应丢失、相同幂等键重试、同键换请求 | 原结果可恢复；不重复创建或发邮件；同键不同请求 409 |
| T03 | 退出 / 解散后旧创建回执或旧已接受邀请重放 | 不恢复名额；204 动作可安全重放 |
...
| T08 | 创建 / 加入请求携带 sharing=false，公开隐私切换 | 仍计入全部用量维度；个人公开配置不影响团队用量 |
| T09 | 加入前后 1 ms、延迟上传 | `occurred_at < joined_at` 不计入；加入后延迟上传计入；离队后延迟上传不计入具名行也不进历史桶 |
| T10 | 退出后再加入同团队 | 新 membershipId、新 joined_at；旧量留在历史用户桶不回挂；加入前个人事件不带入；旧导出不可用 |
...
| T20 | 一人退出后总量与分布仍含其旧量、贡献榜无此人 | 历史用户按日合并；7d/30d 窗口正确；CSV 匿名标签 |
| T21 | SMTP 超时 / 撤销中投递、旧密钥、未知模板、重复请求 | 成员数不变；邮件可失效但接受必拦；不发送明文回退载荷 |
| T22 | 头像超限 / 伪扩展、上传后降权、CSV 公式文本 | 内容验证与完成时授权生效；下载不执行恶意单元格公式 |
| T23 | 创建页错误、并发名额冲突、取消、成功后刷新 | 输入保留；无用量开关；成功恰 1 个成员；刷新不重复创建 |
| T24 | 所有角色、空 / 更新 / 失败态、中英文、360 / 736 / 1024 px | 控件与权限一致；无横向溢出；键盘与焦点操作可用 |
| T25 | 多个不同邮箱账号使用同一分享链接；与邮箱接受 / 创建并发 | 无需指定邮箱匹配；仍一人一队；固定 member；加入即计入 |
| T26 | 链接仅剩一次额度，同时两名新用户接受 | 至多一人成功；成员、消费记录与计数原子一致 |
| T27 | 预览、复制、响应丢失重试、已在队、退出后重试、回执过期 | 不重复扣次数；仍有效链接的消费记录不能因 7 天回执清理而丢失 |
| T28 | 接受与撤销 / 重新生成 / 创建者降权、暂停、解散并发 | 提交顺序裁决；旧链接不再新加成员，已加入关系保留 |
| T29 | 被移除用户再用同一条链接，再用一条新链接 / 新定向邮箱邀请 | 同一链接 410 ALREADY_USED（不是 TEAM_REINVITATION_REQUIRED）；新入口可以建立新 stint |
...
1. Go 领域与 Service 单测覆盖 `joined_at` 边界、角色、Token / 费用 fixture、日期边界；Handler 测试覆盖真实错误结构、CSRF、no-store 与跨团队访问。
...
5. 在 web 目录运行 `npm run typecheck`、`npm test`、`npm run build`；新增团队 E2E 覆盖真实创建 → 邮箱 / 分享链接邀请 → 接受 → 退出，两种路径分别验证。邮件使用测试 Provider，不向真实用户发送验收邮件。
6. 联调在实际 `/token-dance/` 子路径验证登录回跳、页面刷新、API、私有头像、CSV 下载与错误恢复；压测记录数据规模、机器配置、SQL 计划及 p95，核实第 15 节目标。

最终上线门槛：单团队唯一性、所有权不变量、加入即全维度、不回填加入前历史、退出 MERGE 历史用户、离队后不写入、删除屏障、后台任务分代保护和导出鉴权全部通过；页面交互完成且没有用 0 或旧数据掩盖不可用状态。
...
- 当前成员、账号有效性、`occurred_at >= joined_at` 和删除屏障继续生效。不恢复加入之前的个人历史。新旧来源以同一行键去重，使用大整数累计。写出掩码恒为全开。
...
已被同日后续锁定结论覆盖：classification / cost 不再是独立开关；设置页不可关闭基础共享。规则 5 快照仍因规则 6 静态化而失效。见下一节。

## 2026-09-12：成员关系、自动共享与退出留量

与产品稿、静态物化方案对齐（锁定，不再重开）：

1. `user_current_teams` PK(`user_id`) 仍是一人一队名额锁。
2. `team_memberships` 只保留当前 stint。leave/remove DELETE occupancy + membership，不写 `ended_at`/`end_reason`。不为 FK 插入 dummy 历史成员行。
3. `team_sharing_grants` 退出分析资格路径；不要求本方案 DROP。在队即 mask=7。
4. 只统计 `occurred_at >= joined_at`。加入回填已存且满足该谓词的事实，不是加入前个人历史。
5. 退出 MERGE 进按日「历史用户」桶；贡献榜 / 名单不含此人；总量不变。
6. 重入队新 `joined_at`；旧量不回挂。同链接 410 `TEAM_INVITE_LINK_ALREADY_USED`。删除 `TEAM_REINVITATION_REQUIRED`。
7. 离队后延迟上传不写具名行也不写历史桶。
8. 解散删除该队全部静态行（含历史桶）。
9. 账号删除若在队则先 leave MERGE；owner 仍 `TEAM_OWNER_TRANSFER_REQUIRED`。
10. 暂停保持具名，直到 leave/remove。

实现细节（历史桶 UNIQUE、DROP `fk_link_joins_membership`、Worker 锁顺序）以 [静态物化方案](tokendance-teams-static-metrics-v1.md) 为准。
