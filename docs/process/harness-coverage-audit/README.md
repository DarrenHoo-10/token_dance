...
## macOS main 复核（2026-09-13）

基于 main `30ad66d`（0.1.38）复核 #87。用量包装、Codex 模型与请求身份、Skill 配对、缓存构成、费用事件和会话时长使用共享实现，Windows 修复同样适用于 Mac。Mac 的旧安装包仍需升级；跳过 0.1.35 直接升级也会通过保留的重建里程碑重读原始来源，已经删除的原始记录不能恢复。

额外确认 Mac 豆包工作版存在两个发现缺口：数据根目录是 `~/Library/Application Support/DoubaoWork`，其 `Default/IndexedDB` 使用 `chrome_doubaowork-chat_0.indexeddb.leveldb` 和 `chrome_doubaowork-launcher_0.indexeddb.leveldb`。Windows 的 `User Data` 包装目录和 `chrome_doubao-*` 名称不能覆盖这个布局。只读本机复现中，修复前适配器没有采集源；补齐目录及数据库名称后成功读出活动和 Skill。

检测器与读取器现共用已支持的 profile 发现函数和四个数据库名称，保留 Windows 布局。合成 WAL 回归覆盖两种平台的 chat/launcher 来源、重复文件去重、重复轮询不重发和未知 Token 不补零。未读取凭据、未写来源数据库或用户统计，仓库夹具不含真实会话。豆包完整累计 Token 的原有限制不变。

本地验证：Mac 桌面 Rust 315 项通过、11 项需外部条件的测试按原配置忽略；其中豆包本机只读验证另行显式执行并通过。来源发现 10 项通过；前端用量 42 项、更新 16 项及资源构建通过。Windows 和 Mac CI 补充来源发现与缓存布局回归。此次修复未升版本、未发布或替换已安装应用。
