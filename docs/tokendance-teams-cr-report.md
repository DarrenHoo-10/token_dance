...
## 2026-09-12 日期筛选工具栏 UI 检查

范围：总览、成员、用量分析共用日期控件。快捷日期和起止输入靠近排列；自定义模式使用浅色紧凑面板；移除页面重复说明，提示统一由组件提供。保留 URL 参数、选齐自动查询、团队时区、90 天与未来日期限制。

审查：检查日期参数保留/清空、三页缺日期分支、输入 label 与提示关联、英文翻译和移动布局。未发现本次变更的阻断项。没有更改统计、权限或数据库。

验证：TypeScript 检查、22 项团队前端测试通过；Chrome 使用模拟接口检查 1440px、390px、360px 布局，无页面运行时异常和移动端横向溢出。截图在忽略目录 build/test-release/date-toolbar-*.png。这是本地 UI 验证，不能代替测试服务真实登录验收。

开发提交：97df1db；干净 release 构建提交：1e88a1995ee693df140bbedc020c8a4f311f18ed。前端及 Linux API/Worker/migrate/checkproviders/grayscale-sync 构建成功。测试发布未完成：用户级 SSH 凭据认证失败；远端 release 拉取也遇到连接重置。未切换测试版本，未发布生产。发布包和 SHA-256 保存于忽略目录 build/test-release/date-toolbar-build-state.json，恢复连接后须重新核对远端/测试当前版本再发布。


## 2026-09-12 图表优先布局复验

总览和用量分析顺序改为指标卡、图表、日期调整、明细。自定义缺日期时复用上次完整范围继续查询，首次缺日期深链接先查询今天，绝不向 API 发送不完整自定义范围。共享 hook 的权限版本及团队变化仍会清除旧数据。

类型检查及 24 项团队测试通过；新增图表在控件之前、编辑保留数据、权限变化清空数据验证。Chrome 模拟接口预览覆盖 1440/390/360px，无运行时错误或横向溢出；截图 build/test-release/charts-first-*.png 使用合成测试数据，不代表云端真实用量。

开发提交 9f50f7f；干净 release 前端构建 SHA：54a8dca494ad45828f117bad2e3ff71d77bd14bd。SSH 再次认证失败，测试部署尚未完成；前一次日期工具栏发布包不包含本次调整，后续发布必须使用本 SHA 重新打包并核验线上版本。


### 趋势卡片内嵌日期控件

按最新反馈，将总览/用量分析的日期控件移入 Token 趋势卡片底部，以细分隔线衔接图表，移除独立面板装饰及重复提示。成员页独立筛选条不变。新增同卡片包含关系断言；24 项团队测试、类型检查和 release 前端构建通过。Chrome 模拟接口下 1440/390/360px 无运行时错误或横向溢出，截图 build/test-release/chart-integrated-*.png。

开发提交 bcd5f43；干净 release 构建 SHA：30ed853992e0de8d5996cfc310613c6ce1d46ffe。尚未发布测试服务：此前核验的 SSH 认证阻塞未解除；旧包不能用于发布本次 UI，恢复后需按此 SHA 重新打包。


## 2026-09-12 成员数据展示复查

- 日期筛选恢复数据区顶部；总览新增活跃成员人均用量、单日峰值、成员多线趋势、成员占比、贡献排行与迷你趋势、模型构成及每日用量强度。
- 成员趋势在已有授权快照行上按成员/团队自然日汇总，不新增表、事件扫描或逐成员 API 请求。成员身份先过滤再分页，占比分母为完整团队用量；最多返回 90 个自然日，截止快照当日。
- 检查大整数精度、DST 自然日、空白日期补零、快照截止、分页身份过滤、筛选后占比分母。新增后端回归用例通过；HTTP API 包测试通过。
- 前端团队相关 26 项测试及类型检查通过；真实 Chrome + 模拟接口完成桌面/390px/360px 布局检查，无页面横向溢出或运行时错误。预览为合成数据，不代表云端数据。
- 发布限制：当前 SSH 用户级凭据被测试服务器拒绝，尚未上线；待凭据恢复后从干净 release 构建发布。

- 构建记录：release / `2b20d069fb4aa3cb2cc9841d61bda7fc603a9690`，干净工作区的测试前端及 Linux API 构建通过，未部署。
