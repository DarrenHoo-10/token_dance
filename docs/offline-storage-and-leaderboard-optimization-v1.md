...
13. 本地仅保留近 7 天事件明细，更早只长期保存最终聚合数据及必要的采集进度、去重和同步版本元数据。离线或未同步不延长明细保留期；云端补传改用带版本的聚合快照。

本方案更新既有设计中“公开资料且有用量才能参榜”和“上传缓存满则停止采集”的规则。其他权限边界沿用原有设计。

以下三项是必须交付的排行榜要求，适用于首页榜单、完整列表和本人排名行：

| 明确要求 | 实施与验收口径 |
| --- | --- |
| 去掉“持平” | 名次未变化时变化列留空，不显示“持平”、横杠、0 或其他占位标记；真实升降仍显示箭头和变化名次 |
| 无论是否公开数据都参与排行榜 | 参榜查询、分数更新及缓存构建均不得以公开开关过滤用户；切换公开开关不移除成员、不重置分数，不改变参榜人数 |
| 只要注册成功就参与榜单 | 新注册账号自动加入今天、近 7 天、近 30 天和累计榜，无用量按 0 计；不要求绑定采集器、上传过数据、开启主页或补全资料；上线时补齐已有注册账号 |

上述规则对所有正常注册账号生效；注销、删除和封禁仍按账号有效性规则处理。参与全量排名与公开列表只展示前 1000 名是两个独立规则。
...
| `events` | `local_seq`、唯一 `event_id`、Agent、时间、类型、规范化负载、解析版本 | 仅近 7 天的用量明细，不保存聊天正文 |
| `daily_agent_metrics` | 归属分区、日期、Agent、统计版本，复合唯一键 | 长期保留 Token、消息、时长、代码等日汇总 |
| `daily_model_metrics` | 归属分区、日期、Agent、模型、计价类别，复合唯一键 | 长期保留模型用量、已记录费用与可重新估价输入 |
| `daily_skill_metrics` | 归属分区、日期、Agent、Skill 标识，复合唯一键 | 长期保留 Skill 次数、活跃日和可确认的结果 |
| `price_catalog` / `event_costs` | 模型、价格版本、事件 ID、计价来源、金额 | 价格缓存及近 7 天逐事件费用；历史费用转入聚合 |
| `source_bucket_contributions` | 稳定来源/片段身份、归属分区、日期、维度、聚合版本 | 来源级聚合贡献及覆盖水位，用于安全替换重扫结果；不保留逐事件负载 |
| `aggregate_buckets` | 归属分区、日期、schema、单调 revision、内容摘要 | 持久聚合快照身份，修正和重新计价也推进版本 |
| `sync_targets` / `sync_delivery` | 账号/设备绑定代次、聚合桶 ID、待传/已确认 revision、租约及重试时间；目标/桶唯一 | 合并同桶更新，待同步状态不保存每次调用或每版完整副本 |

必要索引：`events(agent_id, occurred_at, local_seq)`、`events(occurred_at, local_seq)`、`sync_delivery(target_id, status, next_attempt_at, bucket_id)`。时间序列界面读取日汇总，不扫描所有事件；补传采用游标分页，不把所有待传聚合桶载入内存。归属分区区分不同账号历史和未绑定历史，不能因按日合并而失去账号隔离。
...
2. 根据事件身份和来源覆盖水位去重；近 7 天写入规范化明细，较早记录解析后直接归入来源聚合，不长期落地事件负载。
3. 更新来源贡献、日汇总、计价输入和聚合桶 revision；纠正旧数据时替换已确认覆盖范围的贡献，不能把重扫结果再次相加。
4. 对启用同步的目标登记该桶最新待传 revision；未登录时也保存聚合，之后按历史归属分批建立聚合桶同步任务。
5. 更新采集进度并提交。
...
- 聚合历史长期保留，事件明细仅保留近 7 天，按 4.5 节清理；上传批次、扫描批次、内存队列和诊断日志分别限额。主数据库容量不采用原上传缓存的阈值。
- 实际磁盘不足、文件权限错误或数据库写入失败时停止提交，保留源进度并提示“存储异常”；空间恢复后从已提交游标继续。
- 日志轮转与 SQLite checkpoint 不负责业务保留策略。明细按保留期清理，长期聚合不随同步 ACK 删除；用户主动删除聚合历史属于独立操作。
...
### 4.5 近 7 天明细与历史聚合保留策略

用户最新确认：本机逐事件去重也仅保留近 7 天，不再以永久逐事件指纹或其压缩作为目标。历史聚合与来源读取进度长期保存。该策略仅作用于本机，不缩短服务端聚合版本/摘要的幂等保护。

实施必须同时处理历史重扫：正常增量读取使用持久化游标；全量读取、轮转或重建历史时，按有效来源覆盖范围生成替换聚合，不能在删除旧指纹后仍把历史记录直接加到已有总量。数据库型来源当前会重复解码整库，因此要先接通覆盖替换，再启用去重过期清理。首次导入历史与之后重扫需要区分，不能用“忽略所有超过 7 天的记录”代替。现有永久指纹实现尚未切换。


按事件发生时间保留今天及之前 6 个 UTC 日的明细，与本方案日聚合边界一致。例如 UTC 9 月 6 日保留 8 月 31 日起的明细，8 月 30 日及更早只保留聚合。截止点由统一时间服务计算，检测到系统时钟异常前跳时暂停清理并校验，不因一次错误时间不可逆地清空近期明细。

- 长期聚合保留“日期 × Agent × 模型”的输入、输出、缓存及其他明确计价类别的 Token、已记录费用、估算费用和覆盖计数；Skill 按日期和标识单独汇总。比例保存分子/分母，平均值保存总量/计数，不能简单累加百分比。非可加指标使用明确的去重或区间合并规则，不把每条事件伪装成一个聚合桶。
- 明细包括逐调用用量和逐事件费用；采集游标、稳定来源身份、覆盖水位、必要的紧凑去重信息和同步版本属于正确性元数据，可随历史保留。禁止借此长期保存全量事件 ID 列表或原始负载；来源级聚合贡献按文件/片段与日维度压缩，并计入容量监控。
- 启动和每日后台维护检查过期范围，有界批次确认聚合、水位及待同步桶已持久化，再清除超期明细和对应逐事件费用。清理与采集串行提交，崩溃后可续跑；不能先删明细再依赖下次任务生成聚合。
- 已同步、未同步、未登录和关闭同步采用同一 7 天策略。断网数月也只积压历史聚合的最新版本，不为等待 ACK 延长明细保留期。未经处理成功的来源不推进游标或丢弃，记录简短错误元数据后从原始来源重试。
- 超过 7 天的迟到记录直接更新所属历史日的聚合及版本，不按发现时间重新保留 7 天明细。已清理历史不代表不可修正，但更改解析口径须按 5.2 节从可用原始来源重建覆盖范围。
- 清理后释放的数据库页优先供后续写入复用；文件体积不保证立即缩小。实施时配置并验收受控的增量空间回收，避免在每轮采集后执行整库压缩。WAL、临时重建文件、日志和隔离诊断均有独立上限及生命周期，不形成第二份永久明细库。

清理不影响已保存的历史趋势、Token、费用或 Skill 汇总；无法再逐条追查旧调用。保留的可加计价输入支持匹配口径下的重新估价；请求级阶梯、最低费用等不能从普通日总量精确推导的规则，需额外保留充分的聚合分组，否则不声称能够精确重算。已删除原始来源的历史无法恢复明细或任意改变统计维度。
...
3. 优先处理活跃文件保证近期数据可见，后台分批补历史；近 7 天落明细，更早流式生成聚合，不能先复制全部原始历史到 SQLite 再清理；进度持久化，重启可续跑。
...
- 同一设备重建保留 installation ID、设备密钥以及已有 `event_id` 算法版本。旧事件通道继续使用 `(installation_id, event_id)` 去重；新聚合通道采用 6.2 节版本替换与覆盖范围切换，不能认为事件 ID 能防止两套通道的重复计分。
...
清理旧事件后不能只靠 `events` 唯一索引去重。正常增量使用稳定来源、片段及已提交游标；轮转/复制识别必须保持来源身份。历史重扫在临时聚合代次完整重算可确认的覆盖范围，再原子替换该范围的来源贡献并发布更高版本；不能把部分仍存在的日志当完整历史覆盖已保存总量。无法确认身份或覆盖完整性的来源须隔离并记录覆盖缺口，不能猜测相加。全量重建只在首次切换且原始来源足够时替换完整范围，后续应保留原始日志已缺失部分的历史聚合。
...
新桌面端统一同步聚合快照，不用“近期传事件、历史再传同一份聚合”的双重计分方式。快照键包括账号归属、installation ID、日期和 schema；内容包含该日范围内完整 Agent/模型/Skill 聚合、单调 revision、内容摘要及明确覆盖范围，数值是绝对值。

- 从 SQLite 按待传桶领取有界批次，捕获一致的 revision 和内容，网络请求在事务外执行。同一桶反复更新合并成最新待传版本，不永久堆积各版请求体。
- 服务端只接受更新版本；同版本同摘要为幂等重试，同版本不同摘要拒绝，旧版本不覆盖新版本。客户端版本在重建/重启后仍须递增；恢复旧备份时先核对服务端版本，不能直接从 1 重置。
- 服务端事务替换该设备、日期覆盖范围的贡献，并标记用户日汇总 dirty；账号总量对互不重复的设备贡献求和。纠错降低分数和删除旧维度都允许通过更高版本完整替换，缺失维度不能残留。大快照可分块暂存，校验完整 manifest 后一次发布，半包不参与排名。
- ACK 必须绑定目标、桶、revision 和摘要。上传 revision 5 期间产生 revision 6，收到旧 ACK 只能确认 5，不能清除 6 的待传状态。ACK 落盘前崩溃允许重传。
- 临时失败退避重试；永久拒收的聚合桶隔离并保留当前聚合和错误元数据，不阻塞其他桶。修复后重发当前版本；不通过保存过期明细等待修复。
- 上传成功只改变同步状态，聚合长期保留，明细仍按 7 天策略清理。未登录期间的聚合按明确历史归属建立同步任务，断网数月后无需旧明细即可补传。
- 历史积压限速并与实时桶公平调度。取消登录和暂停同步有有界响应时间，不一次持锁上传数万桶；积压不阻断本地采集。

协议上线必须配套服务端能力协商和计分来源切换。以设备及日期覆盖范围标记唯一有效来源：旧事件汇总或新聚合快照。只有完整新快照验证成功后，才在事务中替换旧事件贡献，不能把新聚合额外加到已上传事件上；切换后迟到旧客户端事件不得再次计分。原始来源不足以覆盖旧云端历史的范围暂不切换，保留旧贡献并标记缺口。该协议属于本方案必须实施的服务端变更，不能只改客户端删除明细后继续依赖旧事件上传接口。
...
保留现有 `usage_events` 供旧协议兼容，复用日汇总和快照表；新桌面通道以设备聚合为持久事实，不依赖长期逐事件补传。拟增加以下逻辑结构，具体 DDL 在实施阶段提交：

| 结构 | 核心字段/索引 |
| --- | --- |
| `device_daily_aggregates` | 账号、installation、日期、schema、维度；完整快照 revision/摘要、各项绝对值和发布状态 |
| `usage_coverage_sources` | 设备、日期覆盖范围唯一；当前有效计分通道、版本与切换状态，防止旧事件与新聚合重复相加 |
...
旧事件接收事务负责幂等插入，新快照接收事务负责版本校验及完整贡献替换，两者均标记受影响日期。聚合任务仅从各覆盖范围当前有效的来源重算用户/日期，不能同时累加同范围的旧事件与新快照，再根据旧、新日汇总的差额维护周期分数；累计分数不在每个读请求里重扫历史。新协议校验聚合各字段的范围、子项关系、设备归属、日期和 schema；设备签名同样不等于用量绝对真实。

注册事务创建零分参榜身份和对应任务。账号状态变化也生成可靠的成员更新任务。资料修改与排名数字可分开发布，但删除/封禁的可见性不得等待普通资料缓存自然过期。

首次启用新参榜规则时，分页枚举所有正常存量账号，为四个周期幂等补建分数和成员任务；已有用量按真实汇总初始化，没有用量才置 0，不覆盖已有有效分数。补建期间的新注册账号由注册事务覆盖，完成后核对有效账号数与各周期成员数。公开开关的更新只改变详情访问权限，不生成退榜或清零任务。
...
| 阶段 | 主要工作 | 退出条件 |
| --- | --- | --- |
| A：本地持久化 | 增加本地 SQLite Repository、事务采集、长期聚合、7 天明细清理、单实例锁 | 离线与重启验收通过，清理不丢聚合、不依赖上传队列 |
| B：全量重建和同步 | 完整来源发现、断点续扫、价格重建、账号隔离、聚合快照协议和服务端计分来源切换 | 老日志重扫、明细到期、乱序补传、新旧协议并存均不丢失或重复计数 |
...
- `collector/crates/wal-spool` / `uploader`：桌面路径逐步脱离自定义上传 WAL，新增聚合快照协议与版本 ACK；复用可兼容的鉴权、重试组件，不能把旧事件幂等直接当作聚合幂等。其他宿主未切换前保留旧协议实现。
...
通过功能开关逐步切换本地读模型、聚合同步、云端参榜规则和 Redis 查询。服务端先支持聚合快照及旧通道围栏，再发布启用 7 天明细策略的新客户端；异常时保留本地聚合等待补传，不回退为无限保留明细。云端全量参榜与详情权限必须配套上线，先完成隐私文案和 API 访问测试。回退优先保留新 SQLite 数据并回退展示/同步功能，不能直接用读不懂新库的旧客户端覆盖，或重新启用已被替换范围的旧事件计分。
...
| 7 天保留边界 | UTC 日界前后、启动补清理、异常时钟前跳和超期迟到记录；正常情况下仅近 7 天有明细，历史聚合及费用不变 |
| 清理与恢复 | 聚合提交和清理各边界中断，重启后不丢统计、不重复计数；逐事件费用、临时文件和诊断不能绕过保留期 |
| 长期离线 | 离线或未登录 90 天，旧明细照常清理；恢复登录后用聚合补齐，云端与本地有效覆盖范围的总量一致 |
| 聚合快照协议 | 重复/乱序、同版本异摘要、旧 ACK、新版本降分、维度删除、分块缺失、旧备份恢复；只应用完整且有效的新版本 |
| 协议切换 | 同一历史已上传事件后再传聚合、切换后旧客户端迟到、原始覆盖不全；不翻倍、不用不完整快照清零历史 |
| 清理后重扫 | 旧明细不存在时重扫轮转/复制日志，采用来源覆盖替换；原始日志部分缺失时保留不可重建历史，无猜测性累加 |
...
| 存量账号补齐 | 历史私密账号和从未上传的账号均进入四周期；重复补建不重复成员、不清零已有分数；并发注册不漏入榜 |
| 公开开关独立 | 开启/关闭公开前后，参榜资格、分数和人数保持一致；私密详情仍只允许本人访问 |
| 去掉持平 | 首页、完整列表和本人行在变化为 0 时留空，无“持平”、横杠或 0 占位；真实升降展示正确 |
...
监控至少覆盖：SQLite 文件/WAL 大小、明细保留边界和清理延迟、聚合/来源元数据体积、可复用页、提交失败、重建进度、待传/隔离桶数、最老待传年龄、服务器 dirty-day/Outbox 积压、Redis 应用延迟、脚本耗时、热榜年龄、缓存命中率、对账差异、全量重建状态与 API 延迟。本地总量仍随历史天数和聚合维度增长，不能承诺固定磁盘占用；但旧逐调用负载不再无限增长。
