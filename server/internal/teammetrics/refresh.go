...
func projectJoinDayHours(ctx context.Context, tx *sql.Tx, contrib Contributor, date string, joinedAt time.Time) ([]DayRow, error) {
	...
	defer arows.Close()
	type joinActivityInput struct{ agent, code, dur, msgs, userMsgs, codeKnown, durKnown, msgKnown string }
	var joinActivityInputs []joinActivityInput
	for arows.Next() {
		...
		joinActivityInputs = append(joinActivityInputs, joinActivityInput{agent, code, dur, msgs, userMsgs, codeKnown, durKnown, msgKnown})
	}
	if err := arows.Err(); err != nil {
		return nil, err
	}
	arows.Close()
	for _, input := range joinActivityInputs {
		agent, code, dur, msgs, userMsgs, codeKnown, durKnown, msgKnown := input.agent, input.code, input.dur, input.msgs, input.userMsgs, input.codeKnown, input.durKnown, input.msgKnown
		row, err := activityRow(contrib, date, agent, code, dur, msgs, userMsgs, codeKnown, durKnown, msgKnown)
		if err != nil {
			return nil, err
		}
		row.Hourly, err = loadActivityHourly(ctx, tx, contrib.UserID, date, agent, startMs)
		if err != nil {
			return nil, err
		}
		out = append(out, row)
