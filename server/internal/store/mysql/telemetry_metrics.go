...
func ApplyModelMetricDeltaTx(ctx context.Context, tx *sql.Tx, userID, installationID, grain string, bucketStart int64, harnessID string, modelKey uint64, semantics uint16, nowMs int64, d map[string]int64) error {
	...
	if negative {
		...
		args = append(args, userID, installationID, grain, bucketStart, harnessID, modelKey)
		result, err := tx.ExecContext(ctx, "UPDATE telemetry_model_metrics SET "+updates+" WHERE user_id=? AND installation_id=? AND grain=? AND bucket_start=? AND harness_id=? AND model_key=? AND delete_at IS NULL", args...)
		if err != nil {
