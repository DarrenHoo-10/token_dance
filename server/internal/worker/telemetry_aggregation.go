...
func (w *Worker) ProcessTelemetryAggregation(ctx context.Context) (int, error) {
	...
	processed, err := w.backfillSessionExtents(ctx, now.UnixMilli())
	if err != nil {
		return 0, err
	}
...
func (w *Worker) applyTelemetryContribution(
	...
	spanChanged, err := applySessionExtent(ctx, tx, grain, ev, nowMs)
	if err != nil {
		return false, err
	}
...
func applyCostDiff(
	...
	if err := tx.QueryRowContext(ctx, "SELECT HEX(fact_key),fact_revision FROM telemetry_events WHERE id=?", ev.ID).Scan(&cand.FactKey, &cand.Revision); err != nil {
		return err
	}
...
func loadCostFacts(ctx context.Context, tx *sql.Tx, ev *telemetryEventRow, grain string) ([]telemetryagg.CostFact, error) {
	...
	rows, err := tx.QueryContext(ctx, `
		SELECT id, occurred_at, model_key,
		       ...
		       JSON_UNQUOTE(JSON_EXTRACT(payload_json, '$.cost.source')), HEX(fact_key),fact_revision
	...
	for rows.Next() {
		...
		var source string
		if err := rows.Scan(&f.EventRowID, &f.OccurredAt, &f.ModelKey, &f.Currency, &f.Units, &source, &f.FactKey, &f.Revision); err != nil {
			return nil, err
		}
